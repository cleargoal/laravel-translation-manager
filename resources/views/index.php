    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                var group = $(this).val();
                if (group) {
                    window.location.href = '<?php echo action('\Cleargoal\TranslationManager\Controller@getView') ?>/'+$(this).val();
                } else {
                    window.location.href = '<?php echo action('\Cleargoal\TranslationManager\Controller@getIndex') ?>';
                }
            });
...
<header class="navbar navbar-static-top navbar-inverse" id="top" role="banner">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a href="<?php echo action('\Cleargoal\TranslationManager\Controller@getIndex') ?>" class="navbar-brand">
                Translation Manager
            </a>
        </div>
    </div>
...
<div class="container-fluid">
    ...
    <p>
        <?php if(!isset($group)) : ?>
        <form class="form-import" method="POST" action="<?php echo action('\Cleargoal\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <form class="form-find" method="POST" action="<?php echo action('\Cleargoal\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <div class="form-group">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
    ...
    <?php if($group): ?>
        <form action="<?php echo action('\Cleargoal\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
        ...
        <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form" action="<?php echo action('\Cleargoal\TranslationManager\Controller@postTranslateMissing') ?>">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
        ...
        <table class="table">
            ...
            <?php foreach ($translations as $key => $translation): ?>
                <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                    ...
                    <?php if ($deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\Cleargoal\TranslationManager\Controller@postDelete', [$group, $key]) ?>"
                               class="delete-key"
    ...
    <?php else: ?>
        <fieldset>
            ...
            <form  class="form-remove-locale" method="POST" role="form" action="<?php echo action('\Cleargoal\TranslationManager\Controller@postRemoveLocale') ?>" data-confirm="Are you sure to remove this locale and all of data?">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Cleargoal\TranslationManager\Controller@postAddLocale') ?>">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <div class="form-group">
                    <p>
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Cleargoal\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish all translations group? This will overwrite existing language files.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
